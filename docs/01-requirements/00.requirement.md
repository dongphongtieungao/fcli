...
# PrivateGPT Adapter Tool Requirement Document
...
Tài liệu này mô tả yêu cầu xây dựng **PrivateGPT Adapter Tool** (sau này được gọi tắt là PA), một công cụ local viết bằng **Python**, chạy trên máy developer, dùng để chuyển đổi **Private GPT internal API** thành **OpenAI compatible API** để **OpenCode CLI** có thể sử dụng như một model provider trong project mã nguồn.

Quyết định hiện tại cho MVP:

1. **Implementation chính:** Python.
2. **Coding CLI chính:** OpenCode CLI.
3. **Gateway trong MVP:** Không bắt buộc dùng LiteLLM. OpenCode gọi trực tiếp PrivateGPT Adapter qua OpenAI compatible API.
...
```text
OpenCode CLI
  -> PrivateGPT Adapter Local Server, http://127.0.0.1:4100/v1
...
- [Provider SPI Specification](./00.requirement_Provider_SPI_Specification.md) quy định boundary và contract cho các AI provider.
- [PrivateGPT Adapter Architecture v2](./00.requirement_PrivateGPT_Adapter_Architecture_v2.md) mô tả kiến trúc mục tiêu và các thành phần kỹ thuật.
...
| PrivateGPT Adapter | Auth, token refresh, model mapping, agent sync, request mapping, SSE parsing, OpenAI compatible response | Không chạy coding tools, không sửa file, không execute command |
...
Developer runs OpenCode in repository
  -> OpenCode calls PrivateGPT Adapter /v1/chat/completions
...
7. Full OpenAI streaming compatibility unless OpenCode requires it.
8. OpenAI native tool calling unless OpenCode requires it.
9. Image input.
10. Full file upload UI.
11. IntelliJ runtime dependency.
12. IntelliJ AgentLoopRunner.
13. Plugin ToolRegistry runtime.
14. Running shell commands inside adapter.
15. Editing files inside adapter.
16. Persisting full chat history inside adapter.
17. Browser UI automation.
...
4. Add streaming if not implemented in MVP.
...
Streaming response may be implemented if OpenCode requires it.
...
sequenceDiagram
    ...
    participant A as PrivateGPT Adapter
...
12. Adapter returns OpenAI compatible response.
...
7. Whether OpenCode requires streaming.
...
[ ] Có thể giải thích lại kiến trúc: OpenCode CLI -> PrivateGPT Adapter -> PrivateGPT Agent -> PrivateGPT Conversation API -> Gemini 2.5 Pro.
...
```text
OpenCode CLI
  -> PrivateGPT Adapter Python, 127.0.0.1:4100/v1
  -> PrivateGPT Agent API
  -> PrivateGPT Conversation API
  -> Gemini 2.5 Pro
```
...
| 2.1 | 2026-06-22 | Published as the canonical PrivateGPT Adapter requirement; linked the Provider SPI and architecture appendices. |
