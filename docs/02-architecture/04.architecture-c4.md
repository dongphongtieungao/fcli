...
`src/` hiện có một skeleton Python, không phải bằng chứng rằng kiến trúc C4 đã hoàn chỉnh:

| Vùng source | Evidence quan sát | Đánh giá so với kiến trúc mục tiêu |
|---|---|---|
| `src/main.py` | Khởi tạo `FastAPI`, include chat router và expose `/health`. | Khớp một phần API Server; chưa chứng minh các endpoint/auth MVP đầy đủ. |
| `src/api/chat.py` | Có module API chat. | Cần được hoàn thiện theo OpenAI-compatible contract. |
| `src/providers/base/provider.py` | Có base provider boundary. | Phù hợp hướng Provider SPI ở mức sơ khởi. |
| `src/providers/registry/provider_registry.py` | Có `ProviderRegistry` và resolve `PrivateGPTProvider`. | Cần hoàn thiện resolution theo model/capability và không hard-code concrete provider. |
| `src/providers/privategpt/provider.py` | Trả mock response `PrivateGPT`. | Chưa có auth, Agent API, Conversation API hoặc SSE integration. |
| `src/services/agent.py`, `src/clients/http_client.py`, `src/resilience/` | Các vị trí component đã tồn tại. | Cần đối chiếu từng implementation với C3 và requirement trước khi claim runtime capability. |

Trạng thái drift: `docs_ahead_of_code`. Đây là hướng chủ đích trong giai đoạn Spec-First; implementation phải bám C4 và requirement thay vì dùng skeleton hiện có làm source of truth.
...
| API contract/tests | Application spec và tests chưa cụ thể | Cần định nghĩa endpoint/schema và evidence thực thi trước pilot. |
