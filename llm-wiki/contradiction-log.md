...
| 2026-06-22 | `requirements-index` previously described `docs/01-requirements/00.requirement.md` as a generic index while its content defined the PrivateGPT Adapter MVP. | Could have caused agents to treat the adapter scope as draft or generic. | resolved — user designated the file as the published canonical requirement; `requirements-index` is retained only as a compatibility alias |
