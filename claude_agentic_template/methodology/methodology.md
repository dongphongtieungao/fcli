...
This document preserves the existing content of `claude_agentic_template/methodology/methodology.md` and connects it to the repository documentation graph for Agentic Spec First work.
...
> - **Console Applications**: CLI tools, batch processors, daemons
...
- [docs/methodology/d_3.document_strategy_and_creation_order.md](../../docs/methodology/d_3.document_strategy_and_creation_order.md): mô tả cần tạo tài liệu nào, theo thứ tự nào, và tài liệu nào phụ thuộc tài liệu nào.
- [docs/methodology/d_1.quickstarts.md](../../docs/methodology/d_1.quickstarts.md): playbook triển khai nhanh theo sprint đầu hoặc 7 ngày khởi động.
...
- **Spec-First & Agentic Development:** Mọi code, test, tài liệu đều lấy đặc tả làm "ground truth". AI cần các tài liệu mô tả rõ ràng, có cấu trúc để hiểu và generate code chính xác. Dưới đây là hướng dẫn chi tiết cách tạo từng loại tài liệu:
  ...
  - **CLI Specification**: Command-line interface spec (cho console apps)
    - **Cấu trúc**: YAML hoặc Markdown với format: Commands → Arguments → Options → Exit Codes → Output Format
    - **Format**: `docs/04-spec-test/cli-spec.yaml` hoặc `docs/04-spec-test/cli-spec.md`
    - **YAML example**:
      ```yaml
      commands:
        - name: process
      ...
      ```
    - **Lưu ý**: Phải định nghĩa rõ argument types, required/optional, exit codes, output formats
...
**Console/CLI Applications**:
  - [ ] Định nghĩa CLI specification (`docs/04-spec-test/cli-spec.md` hoặc `docs/04-spec-test/cli-spec.yaml`):
    - Commands và subcommands
    - Arguments và options
    - Exit codes
    - Output format (JSON, YAML, text, table)
    - Error handling
  - **Quan trọng**: AI dùng CLI spec để generate command-line interface code.
...
**Console/CLI Applications**:
  - [ ] Yêu cầu AI tạo CLI scaffold dựa trên CLI spec:
    ...
    - **Go**: Cobra, urfave/cli
    - **Rust**: clap
    - Dựa trên **CLI specification** và coding standards
...
**Console/CLI Applications**:
- `docs/04-spec-test/cli-spec.md` hoặc `docs/04-spec-test/cli-spec.yaml` — CLI specification
...
### CLI Specification (cho Console/CLI Applications)

**Mục đích**: Định nghĩa command-line interface, AI generate CLI code đúng chuẩn.

**Format**: Markdown hoặc YAML
```yaml
# docs/04-spec-test/cli-spec.yaml
commands:
  - name: process
    description: Process files
    arguments:
...
**Usage**: `app process <input> [options]`
...
**AI sử dụng**: AI đọc CLI spec và generate:
- **Command parsers**: Click (Python), Picocli (Java), Commander.js (Node.js), System.CommandLine (.NET), Cobra (Go), clap (Rust)
- **Argument validation**: Type checking, required/optional handling
- **Help text**: Auto-generate help messages
- **Error handling**: Exit codes và error messages
...
- Code/tài liệu/test mới phải bám đặc tả:
  ...
  * Console/CLI apps: docs/04-spec-test/cli-spec.md hoặc cli-spec.yaml
...
- Console/CLI apps: CLI spec: docs/04-spec-test/cli-spec.md hoặc cli-spec.yaml
...
- Loại ứng dụng: [Web/Console/Desktop/Mobile - chỉ định rõ]
- Framework: [FastAPI/Spring Boot/Express/ASP.NET Core/Click/Picocli/Tkinter/JavaFX/WPF/Qt - chỉ định rõ]
...
- Console: Tạo CLI với Click/Picocli/Commander.js/System.CommandLine dựa trên CLI spec, sinh tests
...
- ✅ **Theo loại ứng dụng**:
  ...
  - **Console/CLI apps**: CLI specification chi tiết (commands, arguments, options, exit codes)
...
```
Thiết kế architecture cho file processing CLI tool với:
- Loại ứng dụng: Console/CLI
- Tech stack options (chọn một):
  * Python: Click/Typer
  * Java: Picocli
  * Node.js: Commander.js/yargs
...
→ Đưa ra CLI specification (commands, arguments, options, exit codes), module structure
```
...
- Tab 4: ChatGPT Plus (GPT-5.1-Codex) - Phase 3: Debugging Console
...
**Scenario 2**: Build File Processing CLI Tool (Console Application)

**Step 0: R&D (ChatGPT 5.1) - 10 phút**
```
Prompt: "Phân tích yêu cầu và đề xuất solution cho file processing CLI tool:
...
→ Đề xuất:
1. Tech stack phù hợp (CLI framework)
...
→ Thiết kế CLI specification chi tiết:
- Commands và subcommands
- Arguments và options
- Exit codes
...
**Output**: CLI specification chi tiết với commands, arguments, options, exit codes...

**Step 2: Implementation (Composer1) - 20 phút**
- Prompt 1: "Implement main CLI entry point với Click" → Accept code → 15 giây
...
> - **Console/CLI apps**: → Đưa ra CLI specification chi tiết (commands, arguments, options, exit codes), module structure, error handling strategy, implementation roadmap
...
> 
> **Console/CLI Applications**:
> > Tech Stack: [Python Click/Java Picocli/Node.js Commander.js/.NET System.CommandLine/Go Cobra/Rust clap]
> > 
> > Dựa trên architecture đã có và CLI spec, implement:
> > 1. [Commands] với:
> >    - [Arguments và options từ CLI spec]
> >    - [Exit codes handling]
> >    - [Output format support (JSON, YAML, text, table)]
> > 2. [Error handling với proper exit codes]
...
> - Spec: docs/04-spec-test/[10.openapi.yaml|cli-spec.md|ui-spec.md]
